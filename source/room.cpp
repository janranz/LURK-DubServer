...
//                 after_action += fmt::format("[ FIGHTER: {0} | HEALTH: {1} ] vs ",(*p)->cTainer.CHARACTER_NAME,h);
...
//             genMulti = (fast_rand() % (((serverStats::PLAYER_BASE_HEALTH + 1) - ((*p)->cTainer.REGEN) / 2)) + ((*p)->cTainer.REGEN / 2));
//             (*p)->heal_player(genMulti);
//             big_bundle_update();
            
//             // m = fmt::format("PLAYER HEALS:{0} recovers {1} health!\n",(*p)->cTainer.CHARACTER_NAME,genMulti);
